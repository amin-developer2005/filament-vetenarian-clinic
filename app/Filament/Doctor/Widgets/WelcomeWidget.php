...
class WelcomeWidget extends Widget
{
    ...
    public int $bookedSlots {
        get => $this->bookedSlots ?? 0;
        set => $this->bookedSlots = $value;
    }
        ...
        $stats = Appointment::query()
            ...
            ->whereHas('slot', function ($query) use ($today) {
                $query->whereHas('schedule', function ($query) {
                    $query->where('doctor_id', Filament::auth()->id());
    ...
    private function hydrateQuickInfo(): void
    {
        ...
        $this->joinedClinics = collect($user->clinics)->map(
            fn (Clinic $clinic) => $clinic->name
        )->join(', ') ?? __('widgets/dashboard.not_available');
    ...
    public function getStatsCards(): array
    {
        return [
            ...
            [
                'label' => __('widgets/dashboard.stats.free_slots.label'),
                'value' => $this->freeSlots,
                'description' => __('widgets/dashboard.stats.free_slots.description'),
                'borderColor' => 'border-t-rose-500',
                'iconBg' => 'bg-rose-50 dark:bg-rose-500/10',
                'iconColor' => 'text-rose-500 dark:text-rose-400',
                'svg' => '<path stroke-linecap="round" stroke-linejoin="round" d="M9.75 9.75l4.5 4.5m0-4.5l-4.5 4.5M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />',
            ...
            [
                'label' => __('widgets/dashboard.stats.reserved_slots.label'),
                'value' => $this->bookedSlots,
                'description' => __('widgets/dashboard.stats.reserved_slots.description'),
                'borderColor' => 'border-t-slate-400',
                'iconBg' => 'bg-slate-100 dark:bg-slate-500/10',
                'iconColor' => 'text-slate-500 dark:text-slate-400',
                'svg' => '<path stroke-linecap="round" stroke-linejoin="round" d="M22 10.5h-6m-2.25-4.125a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zM4 19.235v-.11a6.375 6.375 0 0112.75 0v.109A12.318 12.318 0 0110.374 21c-2.331 0-4.512-.645-6.374-1.766z" />',
    ...
    private function countFreeSlots(): void
    {
        $today = Carbon::today();

        $this->freeSlots = Slot::query()
     ...
     private function countBookedSlots(): void
     {
        $today = Carbon::today();

        $this->bookedSlots = Slot::query()
            ->whereHas('schedule', function ($query) {
                $query
                    ->where('clinic_id', $this->clinic->getKey())
                    ->where('doctor_id', Filament::auth()->id());
    ...
    {
        return __('widgets/dashboard.empty_schedule.panel.doctor.title');
    }

    public function getEmptyScheduleDescription(): string
    {
        return __('widgets/dashboard.empty_schedule.panel.doctor.description');
    }
}
